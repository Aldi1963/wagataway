...
type ResellerSubUser struct {
	ID         uint      `gorm:"primaryKey" json:"id"`
	ResellerID uint      `gorm:"index;not null" json:"resellerId"`
	...
	Reseller User  `gorm:"foreignKey:ResellerID" json:"-"`
...
type BotProduct struct {
	...
	Stock       int       `gorm:"default:0" json:"stock"` // -1 = unlimited
...
type BotOrder struct {
	...
	Status     string   `gorm:"size:20;default:pending" json:"status"` // pending, confirmed, shipped, completed, cancelled
	Notes      string   `gorm:"type:text" json:"notes"`
	CreatedAt  time.Time `json:"createdAt"`
	UpdatedAt  time.Time `json:"updatedAt"`
